def ler_numero_inteiro(mensagem = "Introduza um valor inteiro:") -> int:
    """
    Função que lê do utilizador um nº inteiro. A função garante que o valor
    inserido é um valor válido.
    """
    while True:
        dados = input(mensagem)
        if len(dados) == 0:
            continue
        # verificar se existe um - no início da str (valor negativo)
        if dados[0] == '-':
...
def ler_numero_inteiro_limites(valor_min,valor_max=None, mensagem = "Introduza um valor inteiro:") -> int:
    ...
    """
    Função que recebe o valor mínimo e máximo a ler do utilizador. A função
...
def ler_numero_decimal(mensagem="Introduza um valor decimal:") -> float:
    """
    Função para ler um nº decimal. A função garante que o valor é válido e eaceita
    como separador das casa decimais . ou ,
    """
    while True:
        dados = input(mensagem)
        if len(dados) == 0:
